...
public class DartBoard_GameManager : MonoBehaviour
{
    ...
    public void DartBoardScoreCheck()
    {
        if (numberOFDartsInScene >= 1)
        {
            ...
            if (_arDart[numberOFDartsInScene - 1].distanceToCenter <= 0.1f)
            {
            ...
            else if (_arDart[numberOFDartsInScene - 1].distanceToCenter <= 0.3f)
            {
                Debug.Log("Good: " + _arDart[numberOFDartsInScene - 1].distanceToCenter);
            }
            else if (_arDart[numberOFDartsInScene - 1].distanceToCenter <= 0.4f)
            {
                Debug.Log("Nice: " + _arDart[numberOFDartsInScene - 1].distanceToCenter);
            }
            else if (_arDart[numberOFDartsInScene - 1].distanceToCenter <= 0.5f)
            {
    ...
    public void PercisionComputation() //Percision Computation
    {
        ...
        for (int i = 0; i <= 4; i++)
        {
            if (distanceToOtherDartAve[i] <= 0.1f)
            {
                percisionRate[i] = 10;
            } 
            else if (distanceToOtherDartAve[i] <= 0.15f)
            {
                percisionRate[i] = 9;
            }
            else if (distanceToOtherDartAve[i] <= 0.2f)
            {
                percisionRate[i] = 8;
            }
            else if (distanceToOtherDartAve[i] <= 0.25f)
            {
                percisionRate[i] = 7;
            }
        ...
        {
            Debug.Log("Exellent Percision: " + currentPercisionScore);
            textPercisionScore.text = "Exellent Percision: " + currentPercisionScore;
        }
        else if (currentPercisionScore >= 40)
        {
            Debug.Log("Very Good Percision: " + currentPercisionScore);
            textPercisionScore.text = "Very Good Percision: " + currentPercisionScore;
        }
        else if (currentPercisionScore >= 35)
        {
            Debug.Log("Good Percision: " + currentPercisionScore);
            textPercisionScore.text = "Good Percision: " + currentPercisionScore;
        }
        else if (currentPercisionScore >= 30)
        {
            Debug.Log("Nice Percision: " + currentPercisionScore);
            textPercisionScore.text = "Nice Percision: " + currentPercisionScore;
        }
        else if (currentAccuracyScore >= 25)
        {
            Debug.Log("Satisfactory Percision: " + currentPercisionScore);
            textPercisionScore.text = "Satisfactory Percision: " + currentPercisionScore;
        }
        else if (currentAccuracyScore >= 20)
        {
            Debug.Log("Lacking Percision: " + currentPercisionScore);
            textPercisionScore.text = "Lacking Percision: " + currentPercisionScore;
        }
    }
