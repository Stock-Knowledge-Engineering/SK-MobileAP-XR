...
public class DartShoot : MonoBehaviour
{
    ...
    void Update()
    {
        if (isAR == true) { //For mobile AR
            /* if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began && canShoot == true)
            {
                //GameObject bullet = Instantiate(projectile, arCamera.position, Quaternion.Euler(new Vector3(90, 0, 0))) as GameObject;
